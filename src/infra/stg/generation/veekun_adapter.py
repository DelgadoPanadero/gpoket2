...
class VeekunAdapter(GenerationAdapter):
    ...
    def __init__(
        self,
        base_dir: Path | str = Path("/workspace/stg/pokemon"),
    ):
    ...
    def save_one(self, generation: int) -> str:
        ...
        file_path = self.base_dir / f"gen{generation}" / f"generation-{generation}.tar.gz"
    ...
    def _sprite_rel_path(self, gen: int, member_path: str) -> str | None:
        ...
        if len(parts) < 4 or parts[0] != "pokemon" or parts[1] != "main-sprites":
            return None
    ...
    def extract_sprites(self) -> Iterator[PokemonEntity]:
        for tar_path in sorted(self.base_dir.glob("**/generation-*.tar.gz")):
